...
sealed trait Server[-R, +E] { self =>
  ...
  private def settings[R1 <: R, E1 >: E](s: Settings[R1, E1] = Settings()): Settings[R1, E1] = self match {
    ...
    case Error(errorHandler)  => s.copy(errorHandler = errorHandler)
  }
...
object Server {
  ...
  private case class Error[R](errorHandler: Option[Throwable => ZIO[R, Nothing, Unit]]) extends Server[R, Nothing]

  def app[R, E](http: HttpApp[R, E]): Server[R, E]                                   = Server.App(http)
  def maxRequestSize(size: Int): UServer                                             = Server.MaxRequestSize(size)
  def port(int: Int): UServer                                                        = Server.Port(int)
  def error[R](errorHandler: Throwable => ZIO[R, Nothing, Unit]): Server[R, Nothing] = Server.Error(Some(errorHandler))
  ...
  def make[R](
    ...
    for {
      ...
      httpH           = ServerRequestHandler(zExec, settings.http, settings.errorHandler)
